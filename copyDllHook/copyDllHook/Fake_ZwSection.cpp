	...
	__in_opt HANDLE FileHandle)
{
	...
	if (bRet && robj.m_FileInfo.bReadDecrypt)
	{
		...
		{
			if (IsAllow(robj) != OpenSucess /*|| (fileResult == OpenOldFileType)*/) // √‹º∂°¢∆Û“µ¬Î°¢º”√‹≤€µ»≈–∂œ
			{
			...
			else
			{
				...
				if (fsi.EndOfFile.QuadPart != 0)
				{
					...
					{
						//OutputLogMsg(LOGLEVEL_ERROR, L"\t\t-[%s][1-5]Handle %d, %s  ß∞‹0x%08x", __FUNCTIONW__, hOldSection, (ObjectAttributes != NULL)? ((ObjectAttributes->ObjectName != NULL)? ObjectAttributes->ObjectName->Buffer: L""): L"", ntStatus);
					...
					if (ntStatus == STATUS_ACCESS_DENIED)
					{
						...
						if (!NT_SUCCESS(ntStatus))
						{
							...
							//OutputLogMsg(LOGLEVEL_ERROR, L"\t\t-[%s][2-5]Handle %d, OldViewBase:%08x  ß∞‹0x%08x", __FUNCTIONW__, hOldSection, pOldViewBase, ntStatus);
					...
					if (!NT_SUCCESS(ntStatus))
					{
						//OutputLogMsg(LOGLEVEL_ERROR, L"\t\t-[%s][3-5]Handle %d, %s  ß∞‹0x%08x", __FUNCTIONW__, hNewSection, (ObjectAttributes != NULL)? ((ObjectAttributes->ObjectName != NULL)? ObjectAttributes->ObjectName->Buffer: L""): L"", ntStatus);
					...
					if (!NT_SUCCESS(ntStatus) || pNewViewBase == NULL)
					{
						...
						// OutputLogMsg(LOGLEVEL_ERROR, L"\t\t-[%s][4-5]Handle %d, NewViewBase:%08x  ß∞‹0x%08x", __FUNCTIONW__, hNewSection, pNewViewBase, ntStatus);
